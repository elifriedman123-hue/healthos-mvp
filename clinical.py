...
# --- 7. CHART ENGINE (FLATTENED LAYERS) ---
...
def plot_chart(marker, results, events, master):
    ...
    # 2. EVENT LAYERS (Independent Scale)
    if not events.empty:
        ...
        # DOTTED LINES (Shared X, but independent Y for pixel perfection)
        ...
        # BOXES (Independent Y Scale 0-10)
        ...
        # WE MUST RETURN A LAYERED CHART WITH RESOLVE_SCALE
        return alt.layer(*layers, ev_box, ev_txt).resolve_scale(y='independent').properties(height=320, background='transparent').configure_view(strokeWidth=0)
