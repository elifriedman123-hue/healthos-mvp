...
def plot_clinical_trend(marker_name, results_df, events_df, master_df):
    ...
    if chart_data.empty: return "EMPTY", 0
    ...
    return final.properties(height=320, background='transparent').configure_view(strokeWidth=0), len(chart_data)
...
# MODE 2: TRENDS
    ...
    for m in selected_markers:
        ...
        if chart == "EMPTY":
            st.warning(f"⚠️ No readable data for {m} (Found {raw_rows} raw entries, but they may be non-numeric).")
        elif chart: 
